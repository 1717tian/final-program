...
def transform_station_data(df, scaler):
...
def inverse_pm25_values(scaler, values):
...
def predict_future_direct7(df, model, scaler):
...
def predict_history_direct7(df, model, scaler):
    data = transform_station_data(
        df,
        scaler
    )
...
def save_result(result, station_name):
    os.makedirs(
        PREDICTION_DIR,
        exist_ok=True
    )
    ...
    result_path = os.path.join(
        PREDICTION_DIR,
        f"{file_name}_warning_direct7_future.csv"
    )
    ...
    print(
        f"预警模型预测结果已保存到: {result_path}"
    )


def save_figure(station_name):
    os.makedirs(
        PREDICTION_DIR,
        exist_ok=True
    )
    ...
    figure_path = os.path.join(
        PREDICTION_DIR,
        f"{file_name}_warning_direct7_prediction.png"
    )
    ...
    print(
        f"预警模型预测图已保存到: {figure_path}"
    )


class PM25WarningPredictApp:
    ...
    def __init__(
        ...
        self.root.title(
            "长春 PM2.5 预警导向特征增强 Direct7 预测系统"
        )
    ...
    def draw_result(
        ...
        self.ax1.set_title(
            f"{station_name} - 预警导向模型未来7天PM2.5预测"
        )
